...
public abstract class MixinImageBufferDownload {
        ...
        if (image == null) {
            ...
            if (SkyHanniMod.getFeature().misc.transparentSkin) {
                ...
                if (image.getHeight() == 32) {
                    skyhanni$drawOriginalImage(bufferedImg, graphics);
                    return skyHanni$drawTransparency(bufferedImg, graphics);
                }
                return image;
            } else {
                this.imageWidth = 64;
                ...
                if (image.getHeight() == 32) {
                    skyhanni$drawOriginalImage(bufferedimage, graphics);
                }
                return skyHanni$drawTransparency(bufferedimage, graphics);
            }
        }
    }

    @Unique
    private BufferedImage skyHanni$drawTransparency(BufferedImage bufferedImg, Graphics graphics) {
        graphics.dispose();
    ...
    @Unique
    private void skyhanni$drawOriginalImage(BufferedImage bufferedImg, Graphics graphics) {
